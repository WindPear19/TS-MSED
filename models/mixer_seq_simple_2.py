...
from models.MSAA import SpatialAttentionModule
...
class MixerModel(nn.Module):
    def __init__(
        ...
        self.S357CNN = S357CNN(in_features=768, kernel_num=100, kernel_sizes=[3], CNN_dropout=0.2)
        self.attention = MLAttention(hidden_size=768)
    ...
    def forward(self, input_ids, inference_params=None):
        ...
        #S1
        hidden_states = self.S357CNN(hidden_states)

        #S2
        masks = torch.ones_like(hidden_states)  # Assuming masks are binary here (can be adjusted as needed)
        hidden_states = self.attention(hidden_states, masks)
